...
def init_wcscorr(input, force=False):
    ...
    if not isinstance(input, fits.HDUList):
        # input must be a filename, so open as FITS object
    ...
    # tell the user if FITS detects any problems with the file as a whole
    fimg.flush('warn')
...
def create_wcscorr(descrip=False, numrows=1, padding=0):
    ...
    # fits.
    # TODO: Make sure this is fixed in fits, then remove this
    newtab.name = 'WCSCORR'
...
def delete_wcscorr_row(wcstab,selections=None,rows=None):
    ...
    ----------
    wcstab: object
        FITS binTable object for WCSCORR table
    selections: dict
